...
class SimpleCache:
    def __init__(self):
        ...
        self._timestamps = {}
        self._lock = Lock()
        self.default_ttl = 30  # 30 seconds default TTL
...
def get_cached_active_vehicles():
    ...
    vehicles = (
        Vehicle.query.filter_by(status="Active").order_by(Vehicle.plate_number).all()
    )
...
def get_cached_assistants():
    ...
    assistants = (
        Manpower.query.filter_by(role="Assistant").order_by(Manpower.name).all()
    )
...
def view_data():
    ...
    pagination = query.order_by(
        Data.due_date.desc().nulls_last(), Data.id.desc()
...
def view_scheduled_data():
    ...
    pagination = query.order_by(Data.id.desc()).paginate(
        page=page, per_page=per_page, error_out=False
    )

    return render_template(
        "view_scheduled_data.html",
        data=pagination.items,
        pagination=pagination,
        search_term=search_term,
...
def download_csv_template():
    ...
    return Response(
        ...
        mimetype="text/csv",
        headers={
            "Content-Disposition": "attachment; filename=data_upload_template.csv"
        },
    )
...
def upload_data():
    if request.method == "POST":
        ...
        try:
            ...
            if not decoded:
                flash(
                    "File encoding error. Unable to read the CSV file. Please ensure it's saved as UTF-8, Latin-1, or Windows-1252 encoding.",
                    "error",
                )
                return redirect(request.url)
            ...
            for row_num, row in enumerate(all_rows, start=2):
                try:
                    ...
                    document_number = (
                        row["Document Number"] if row["Document Number"] else None
                    )

                    # Validate posting_date - NOT NULL constraint
                    if posting_date is None:
                        if document_number:
                            posting_date_errors.append(
                                f"Row {row_num}: Missing posting_date (Document Number: {document_number})"
                            )
                        else:
                    ...
                    validated_records.append(
                        {
                            ...
                            "remaining_open_qty": safe_float(
                                row["Remaining Open Qty"], 0.0
                            ),
            ...
            csv_keys = [
                (r["document_number"], r["item_number"], r["ordered_qty"])
                ...
                with db.session.no_autoflush:
                    for record in records_to_insert:
                        data_entry = Data(
                            ...
                            cbm=record["cbm"],
                            total_cbm=round(
                                record["cbm"] * record["ordered_qty_float"], 2
                            ),
...
def edit_data(id):
    ...
    if request.method == "POST":
        try:
            ...
            data.posting_date = (
                datetime.strptime(posting_date_str, "%Y-%m-%d").date()
                if posting_date_str
                else None
            )
            ...
            data.remaining_open_qty = (
                float(request.form["remaining_open_qty"])
                if request.form["remaining_open_qty"]
                else 0.0
            )
            ...
            if due_date_changed:
                ...
                related_records = Data.query.filter(
                    Data.document_number == document_number, Data.id != id
                ).all()

                # Update only their due_date (preserve original_due_date for DIFOT accuracy)
                for record in related_records:
                    record.due_date = new_due_date
                    # NOTE: original_due_date is NOT updated to preserve DIFOT calculation accuracy
                    updated_count += 1
            ...
            if updated_count > 0:
                flash(
                    f"Record updated successfully! Also updated due date for {updated_count} other record(s) with document number {data.document_number}",
                    "success",
                )
            else:
...
def soft_delete_data():
    ...
    try:
        ...
        if not delete_remarks:
            return jsonify(
                {"success": False, "message": "Delete remarks are required"}
            ), 400
        ...
        db.session.rollback()
        return jsonify(
            {"success": False, "message": f"Error deleting record: {str(e)}"}
        ), 500
...
def upload_clusters():
    if request.method == "POST":
        ...
        try:
            ...
            if missing:
                flash(
                    f"Invalid CSV headers. Missing columns: {', '.join(missing)}",
                    "error",
                )
                return redirect(request.url)
            ...
            invalidate_reference_cache()  # Invalidate cluster cache
            flash(
                f"Successfully uploaded {records_added} cluster(s)! All previous data was replaced.",
                "success",
            )
            return redirect(url_for("manage_clusters"))
...
def add_user():
    ...
    try:
        ...
        user = User(name=name, email=email, position=position, status=status)
    ...
    if request.method == "POST":
        try:
            ...
            existing_user = User.query.filter(
                User.email == user.email, User.id != id
            ).first()
            if existing_user:
                flash("Email already exists", "error")
                return redirect(url_for("manage_users"))
...
def generate_user_report():
    ...
    try:
        ...
        db.session.commit()
        flash(
            "All user passwords have been reset. New passwords are included in the CSV report."
        )
    except Exception as e:
        db.session.rollback()
...
def view_schedule():
    ...
    if current_user.position == "admin":
        ...
        if user_manpower:
            ...
            prior_incomplete_trips = (
                Trip.query.join(Schedule, Trip.schedule_id == Schedule.id)
                .join(TripDetail, Trip.id == TripDetail.trip_id)
                .filter(
                    db.and_(
            ...
            all_schedule_ids = list(
                set(
                    [trip.schedule_id for trip in todays_trips]
    ...
    for schedule in schedules:
        for trip in schedule.trips:
            trip.details = sorted(
                trip.details,
                key=lambda d: (d.delivery_order is None, d.delivery_order or 999),
            )

    # Fetch executive vehicles for the refill form
    executive_vehicles = Vehicle.query.filter_by(
        dept="Executive", status="Active"
    ).all()
...
def add_schedule():
    ...
    if request.method == "POST":
        try:
            delivery_date = datetime.strptime(
                request.form["delivery_schedule_from"], "%Y-%m-%d"
            ).date()
            ...
            for i in range(1, trip_count + 1):
                ...
                # Validate: vehicle is required (driver and assistant are optional)
                if not vehicle_id:
                    continue
                ...
                delivery_orders_str = request.form.get(
                    f"trip_{i}_delivery_orders", "{}"
                )
                try:
                    delivery_orders = (
                        json.loads(delivery_orders_str) if delivery_orders_str else {}
                    )
                ...
                for data_id in data_ids:
                    ...
                    if data:
                        branch_name = (
                            data.branch_name_v2 or data.branch_name or "Unknown"
                        )
                        ...
                        branch_groups[branch_name]["document_numbers"].append(
                            data.document_number
                        )
                        branch_groups[branch_name]["total_cbm"] += (
                            data.cbm * data.ordered_qty or 0.0
                        )
                        branch_groups[branch_name]["total_ordered_qty"] += (
                            data.ordered_qty or 0
                        )

                        # Set delivery_order from first data_id if not already set
                        if (
                            branch_groups[branch_name]["delivery_order"] is None
                            and str(data.id) in delivery_orders
                        ):
                            branch_groups[branch_name]["delivery_order"] = (
                                delivery_orders[str(data.id)]
                            )
                ...
                for branch_name, branch_data in branch_groups.items():
                    ...
                    detail = TripDetail(
                        document_number=branch_data["document_numbers"][
                            0
                        ],  # Store first doc number for reference
                        branch_name_v2=branch_name,
                        ...
                        total_delivered_qty=branch_data[
                            "total_ordered_qty"
                        ],  # Initialize with total_ordered_qty
                        status="Delivered",
                        original_due_date=branch_data[
                            "original_due_date"
                        ],  # Save original due date
            ...
            if schedule.trips:
                ...
                if first_trip.vehicle:
                    ...
                    schedule.type = (
                        first_trip.vehicle.type
                    )  # Save vehicle type to schedule
    ...
    return render_template(
        "add_schedule.html", vehicles=vehicles, drivers=drivers, assistants=assistants
    )
...
def api_not_scheduled():
    ...
    if due_date_from_str:
        ...
        if due_date_to_str:
            ...
            subq = Data.query.filter(
                Data.status == "Not Scheduled", Data.due_date == from_date
            ).subquery()
    elif due_date_str:
        # Backward compatibility with old parameter
        due_date = datetime.strptime(due_date_str, "%Y-%m-%d").date()
        subq = Data.query.filter(
            Data.status == "Not Scheduled", Data.due_date == due_date
        ).subquery()
    else:
        return jsonify([])
    ...
    results = (
        db.session.query(
            ...
            func.group_concat(subq.c.id).label(
                "data_ids"
            ),  # Collect all IDs for this doc
        )
        .group_by(subq.c.document_number)
        .all()
    )
        ...
        documents.append(
            {
                ...
                "original_due_date": row.original_due_date.strftime("%Y-%m-%d")
...
def get_areas():
    ...
    areas = (
        db.session.query(Cluster.area)
        .filter(Cluster.area.isnot(None))
        .filter(Cluster.area != "")
        .distinct()
        .order_by(Cluster.area)
        .all()
    )
...
def api_vehicle_schedule_status():
    ...
    try:
        ...
        existing_schedules = (
            db.session.query(Schedule, Trip, Vehicle)
            .join(Trip, Schedule.id == Trip.schedule_id)
...
def api_available_for_edit():
    ...
    existing_trip_details = (
        TripDetail.query.join(Trip).filter(Trip.schedule_id == schedule_id).all()
    )
...
def add_shipments_to_trip():
    ...
    try:
        ...
        for data_id in data_ids:
            ...
            if data_record:
                branch_name = (
                    data_record.branch_name_v2 or data_record.branch_name or "Unknown"
                )
                if branch_name not in branch_groups:
                    branch_groups[branch_name] = {
                        "data_ids": [],
                        ...
                        "total_cbm": 0.0,
                        "total_ordered_qty": 0,
                        "area": data_record.branch_name
                        or data_record.branch_name_v2
                        or "",
                ...
                branch_groups[branch_name]["document_numbers"].append(
                    data_record.document_number
                )
                branch_groups[branch_name]["total_cbm"] += (
                    data_record.cbm * data_record.ordered_qty or 0.0
                )
                branch_groups[branch_name]["total_ordered_qty"] += (
                    data_record.ordered_qty or 0
                )
        ...
        max_order_result = (
            db.session.query(db.func.max(TripDetail.delivery_order))
        ...
        return jsonify(
            {
                "success": True,
                "message": f"Successfully added {len(data_ids)} shipment(s) to trip",
            }
        )
...
def get_trip_shipments(trip_id):
    """Get all trip details (grouped shipments) currently assigned to a trip"""
    if current_user.position != "admin":
    ...
    try:
        ...
        for detail in trip.details:
            # Get the first data record to extract document info
            first_data_record = None
            if detail.data_ids:
                data_id_list = detail.data_ids.split(",")
                first_data_record = db.session.get(Data, data_id_list[0])

            # Count how many line items in this grouped detail
            line_item_count = len(detail.data_ids.split(",")) if detail.data_ids else 0

            shipments.append(
                {
                    "trip_detail_id": detail.id,
                    "data_ids": detail.data_ids,
                    "document_number": first_data_record.document_number
                    ...
                    "due_date": first_data_record.due_date.strftime("%Y-%m-%d")
                    if first_data_record and first_data_record.due_date
                    else None,
...
def remove_shipment_from_trip():
    """Remove a trip detail (grouped shipment) from a trip"""
    if current_user.position != "admin":
        return jsonify(
            {"success": False, "message": "Access denied. Admin privileges required."}
        ), 403
    ...
    try:
        data = request.get_json()
        trip_id = data.get("trip_id")
        trip_detail_id = data.get("trip_detail_id")

        if not trip_id or not trip_detail_id:
            return jsonify(
                {"success": False, "message": "Missing required parameters"}
            ), 400
        ...
        trip_detail = db.session.get(TripDetail, trip_detail_id)
        if not trip_detail or trip_detail.trip_id != trip.id:
            return jsonify(
                {"success": False, "message": "Trip detail not found in this trip"}
            ), 404

        # Store info for response message
        branch_name = trip_detail.branch_name_v2
        total_cbm = trip_detail.total_cbm

        # Mark all associated data records as Not Scheduled
        if trip_detail.data_ids:
            data_id_list = trip_detail.data_ids.split(",")
            data_records = (
                db.session.query(Data).filter(Data.id.in_(data_id_list)).all()
            )
            for data_record in data_records:
                data_record.status = "Not Scheduled"
                data_record.delivered_qty = 0

        # Remove the entire TripDetail
        ...
        # Update schedule actual total CBM
        if trip.schedule:
            trip.schedule.actual = sum(
                t.total_cbm for t in trip.schedule.trips if t.total_cbm
            )
        ...
        return jsonify(
            {
                "success": True,
                "message": f"Successfully removed {branch_name} ({total_cbm:.2f} CBM) from trip",
            }
        )
...
def toggle_trip_complete():
    ...
    try:
        ...
        return jsonify(
            {
                "success": True,
                "message": f"Trip #{trip.trip_number} {status}",
...
def get_trip_crew(trip_id):
    ...
    try:
        ...
        all_drivers = (
            Manpower.query.filter_by(role="Driver").order_by(Manpower.name).all()
        )
        all_assistants = (
            Manpower.query.filter_by(role="Assistant").order_by(Manpower.name).all()
        )
        ...
        return jsonify(
            {
                ...
                "all_assistants": [
                    {"id": a.id, "name": a.name} for a in all_assistants
                ],
            }
        )
            ...
            details.append(
                {
                    ...
                    "delivery_order": detail.delivery_order,
                    "total_cbm": detail.total_cbm,
                    "arrive": detail.arrive.strftime("%Y-%m-%dT%H:%M")
                    if detail.arrive
                    else None,
                    "departure": detail.departure.strftime("%Y-%m-%dT%H:%M")
                    if detail.departure
                    else None,
                }
            )
    ...
    try:
        ...
        for detail_id_str, order in orders.items():
            ...
            if detail and detail.trip_id == trip_id:
                ...
                print(
                    f"Warning: Detail {detail_id} not found or not part of trip {trip_id}"
                )
        ...
        return jsonify(
            {"success": False, "message": f"Error updating delivery order: {str(e)}"}
        ), 500
...
def update_trip_crew():
    ...
    try:
        ...
        if not driver_ids:
            return jsonify(
                {"success": False, "message": "At least one driver is required"}
            ), 400
        ...
        db.session.rollback()
        return jsonify(
            {"success": False, "message": f"Error updating crew: {str(e)}"}
        ), 500
...
def update_trip_vehicle():
    ...
    try:
        ...
        if not trip_id or not vehicle_id:
            return jsonify(
                {"success": False, "message": "Trip ID and Vehicle ID are required"}
            ), 400
        ...
        db.session.rollback()
        return jsonify(
            {"success": False, "message": f"Error updating vehicle: {str(e)}"}
        ), 500
...
def update_trip_times():
    ...
    try:
            ...
            if arrive:
                try:
                    ...
                    return jsonify(
                        {
                            "success": False,
                            "message": f"Invalid arrive time format for detail {detail_id}",
                        }
                    ), 400
            elif arrive is None:
                detail.arrive = None
            ...
            if departure:
                try:
                    ...
                    return jsonify(
                        {
                            "success": False,
                            "message": f"Invalid departure time format for detail {detail_id}",
                        }
                    ), 400
        ...
        db.session.rollback()
        return jsonify(
            {"success": False, "message": f"Error updating times: {str(e)}"}
        ), 500
...
@app.route("/update_odo_log", methods=["POST"])
@login_required
def update_odo_log():
    """Update an existing odometer log"""
    if current_user.position != "admin":
        return jsonify(
            {"success": False, "message": "Access denied. Admin privileges required."}
        ), 403
    ...
    try:
        ...
        price_per_litter = data.get("price_per_litter")

        if not odo_id:
            return jsonify({"success": False, "message": "ODO log ID is required"}), 400

        odo = db.session.get(Odo, odo_id)
        if not odo:
            return jsonify({"success": False, "message": "ODO log not found"}), 404

        # Update fields
        odo.status = status
        odo.odometer_reading = float(odometer_reading)
        odo.litters = float(litters) if litters else None
        odo.amount = float(amount) if amount else None
        odo.price_per_litter = float(price_per_litter) if price_per_litter else None

        db.session.commit()
        return jsonify({"success": True, "message": "Odometer log updated successfully"})

    except Exception as e:
        db.session.rollback()
        return jsonify({"success": False, "message": f"Error updating ODO log: {str(e)}"}), 500
...
def odo_logs():
    ...
    pagination = query.order_by(Odo.datetime.desc()).paginate(
        page=page, per_page=per_page, error_out=False
    )
...
def dashboard_kpis():
    ...
    manila_now = datetime.now(manila_tz).date()
    ...
    # Ensure end_date is not in the future (Manila time)
    if end_date > manila_now:
        end_date = manila_now
    ...
    response = {
        "on_time_delivery_rate": {
            ...
            "trend": calculate_trend(
                kpis["on_time_rate"], previous_kpis["on_time_rate"]
            ),
        ...
        "in_full_delivery_rate": {
            ...
            "trend": calculate_trend(
                kpis["in_full_rate"], previous_kpis["in_full_rate"]
            ),
        ...
        "fuel_efficiency": {
            ...
            "trend": calculate_trend(
                kpis["km_per_liter"], previous_kpis["km_per_liter"]
            ),
        ...
        "data_completeness": {
            ...
            "trend": calculate_trend(
                kpis["completeness"], previous_kpis["completeness"]
            ),
...
def calculate_period_kpis(start_date, end_date):
    ...
    on_time_rate = (
        (on_time_details / total_details_with_due * 100)
        if total_details_with_due > 0
        else 0
    )

    # In-Full Delivery Rate (in-house only)
    from sqlalchemy import func as sql_func

    total_delivered = (
        db.session.query(sql_func.coalesce(sql_func.sum(TripDetail.total_delivered_qty), 0))
        .join(Trip)
        .join(Schedule)
        .filter(
            Schedule.delivery_schedule.between(start_date, end_date),
    ...
    total_ordered = (
        db.session.query(sql_func.coalesce(sql_func.sum(TripDetail.total_ordered_qty), 0))
        .join(Trip)
        .join(Schedule)
        .filter(
            Schedule.delivery_schedule.between(start_date, end_date),
    ...
    in_full_rate = (total_delivered / total_ordered * 100) if total_ordered > 0 else 0

    # Total trip details (for data completeness calculation)
    total_details = (
        db.session.query(TripDetail)
        .join(Trip)
        .join(Schedule)
    ...
    for vehicle in db.session.query(Vehicle).filter(Vehicle.status == "Active").all():
        ...
        for reading in odo_readings:
            if reading.status == "start odo":
                last_start_odo = reading.odometer_reading
            elif reading.status == "end odo" and last_start_odo is not None:
                distance = reading.odometer_reading - last_start_odo
                if distance > 0:
                    total_km += distance
                last_start_odo = None
    ...
    if total_distance > 0:
        km_per_liter = (
            sum([d["km_per_liter"] * d["distance"] for d in fuel_efficiency_data])
            / total_distance
        )
    else:
        km_per_liter = 0
...
def dashboard_trends():
    ...
    while current_date <= end_date:
        count = (
            ...
            .join(Trip)
            .join(Schedule)
            .filter(Schedule.delivery_schedule == current_date)
            .count()
        )
        delivery_counts.append({"date": current_date.isoformat(), "count": count})
        current_date += timedelta(days=1)

    # Fuel efficiency trend (calculate daily)
    fuel_efficiency = []
    current_date = start_date
    while current_date <= end_date:
        ...
        # Calculate daily fuel efficiency
        daily_fe_data = []
        for vehicle in (
            db.session.query(Vehicle).filter(Vehicle.status == "Active").all()
        ):
            odo_readings = (
                db.session.query(Odo)
                .filter(
        ...
        if total_distance > 0:
            daily_km_per_liter = (
                sum([d["km_per_liter"] * d["distance"] for d in daily_fe_data])
                / total_distance
            )
        else:
...
def dashboard_comparisons():
    ...
    # Support "today" query parameter for single-day view
    if request.args.get("today") == "true":
        start_date = manila_now
        end_date = manila_now
    ...
    vehicle_util = (
        ...
        .group_by(Vehicle.plate_number, Vehicle.capacity)
        .all()
    )
    ...
    for i, v in enumerate(
        sorted(
            vehicle_util,
            key=lambda x: (x.total_cbm / x.capacity * 100) if x.capacity else 0,
            reverse=True,
        )
    ):
    ...
    branch_counts = (
        ...
        .join(Trip)
        .join(Schedule)
        .filter(Schedule.delivery_schedule.between(start_date, end_date))
        .group_by(TripDetail.branch_name_v2)
    ...
    # Driver/assistant performance with enhanced metrics
    def calculate_person_performance(person_name, role, start_date, end_date):
        """Calculate comprehensive performance metrics for a driver/assistant"""

        # Get all trip IDs for this person within date range
        if role == "driver":
            trip_ids = (
        ...
        trip_ids = [t[0] for t in trip_ids]

        if not trip_ids:
            return {
                "trips": 0,
                "drop_completion_rate": 0.0,
                "total_drops": 0,
                "completed_drops": 0,
                "odo_compliance_rate": 0.0,
                "compliant_days": 0,
                "total_active_days": 0,
            }

        # 1. Calculate Drop Completion Rate
        # Get all trip details for these trips
        trip_details = (
            db.session.query(TripDetail).filter(TripDetail.trip_id.in_(trip_ids)).all()
        )

        total_drops = len(trip_details)
        completed_drops = sum(1 for td in trip_details if td.arrive and td.departure)
        drop_completion_rate = (
            (completed_drops / total_drops * 100) if total_drops > 0 else 0.0
        )

        # 2. Calculate ODO Compliance Rate
        # Get unique dates and vehicles for this person's trips
        trip_info = (
            db.session.query(Schedule.delivery_schedule, Vehicle.plate_number)
            .join(Trip, Schedule.id == Trip.schedule_id)
            .join(Vehicle, Trip.vehicle_id == Vehicle.id)
        ...
        total_active_days = len(trip_info)
        compliant_days = 0

        for trip_date, plate_number in trip_info:
            # Check if vehicle has required ODO records for this date
            odo_records = (
                db.session.query(Odo.status)
                .filter(
                    Odo.plate_number == plate_number,
                    sql_func.date(Odo.datetime) == trip_date,
                )
                .all()
            )

            statuses = [o[0] for o in odo_records]

            # Check compliance: need start odo, end odo, and at least 1 refill odo
            has_start = "start odo" in statuses
            has_end = "end odo" in statuses
            has_refill = statuses.count("refill odo") >= 1

            if has_start and has_end and has_refill:
                compliant_days += 1

        odo_compliance_rate = (
            (compliant_days / total_active_days * 100) if total_active_days > 0 else 0.0
        )

        return {
            "trips": len(trip_ids),
            "drop_completion_rate": round(drop_completion_rate, 1),
            "total_drops": total_drops,
            "completed_drops": completed_drops,
            "odo_compliance_rate": round(odo_compliance_rate, 1),
            "compliant_days": compliant_days,
            "total_active_days": total_active_days,
        }

    # Get list of all drivers and assistants in the date range
    driver_names = (
        db.session.query(Manpower.name)
        .join(trip_driver, Manpower.id == trip_driver.c.manpower_id)
        .join(Trip, Trip.id == trip_driver.c.trip_id)
        .join(Schedule)
        .filter(Schedule.delivery_schedule.between(start_date, end_date))
        .distinct()
        .all()
    ...
    assistant_names = (
        db.session.query(Manpower.name)
        .join(trip_assistant, Manpower.id == trip_assistant.c.manpower_id)
        .join(Trip, Trip.id == trip_assistant.c.trip_id)
        .join(Schedule)
        .filter(Schedule.delivery_schedule.between(start_date, end_date))
        .distinct()
        .all()
    ...
    for (name,) in driver_names:
        metrics = calculate_person_performance(name, "driver", start_date, end_date)
        metrics["name"] = name
        metrics["role"] = "driver"
        all_performance.append(metrics)

    for (name,) in assistant_names:
        metrics = calculate_person_performance(name, "assistant", start_date, end_date)
        metrics["name"] = name
        metrics["role"] = "assistant"
        all_performance.append(metrics)

    # Sort by trip count (primary), then by drop completion rate (secondary)
    all_performance_sorted = sorted(
        all_performance,
        key=lambda x: (x["trips"], x["drop_completion_rate"]),
        reverse=True,
    )
...
def time_logs():
    ...
    if current_user.position == "admin":
        active_time_logs = (
            active_query.filter_by(time_out=None).order_by(TimeLog.time_in.desc()).all()
        )
    else:
        active_time_logs = (
            active_query.filter_by(time_out=None).order_by(TimeLog.time_in.desc()).all()
    ...
    return render_template(
        "time_logs.html", time_logs=time_logs, active_time_logs=active_time_logs
    )
    ...
    try:
        ...
        if current_user.position == "admin":
            ...
            time_log = TimeLog.query.filter_by(
                id=time_log_id, user_id=current_user.id
            ).first_or_404()
        ...
        if time_log.daily_rate and time_log.sched_start and time_log.sched_end:
            try:
                ...
                lunch_start = effective_start.replace(
                    hour=12, minute=0, second=0, microsecond=0
                )
                lunch_end = effective_start.replace(
                ...
                scheduled_hours = (
                    sched_end_actual - sched_start_actual
                ).total_seconds() / 3600
                ...
                hourly_rate = (
                    time_log.daily_rate / 8
                )  # Assuming 8-hour workday for daily rate
        ...
        if current_user.position == "admin" and time_log.user_id != current_user.id:
            flash(
                f"Successfully timed out {time_log.user.name} at {time_out.strftime('%I:%M %p')}! Hours rendered: {time_log.hrs_rendered}",
                "success",
            )
        else:
            flash(
                f"Successfully timed out at {time_out.strftime('%I:%M %p')}! Hours rendered: {time_log.hrs_rendered}",
                "success",
            )
    except Exception as e:
        db.session.rollback()
        ...
        try:
            ...
            if time_log.time_out:
                if time_log.daily_rate and time_log.sched_start and time_log.sched_end:
                    try:
                        # Parse schedule times
                        sched_start_dt = datetime.strptime(
                            time_log.sched_start, "%H:%M"
                        )
                        sched_end_dt = datetime.strptime(time_log.sched_end, "%H:%M")
                        ...
                        # Check if the work period crosses lunch time
                        crosses_lunch = (
                            effective_start < lunch_end
                            and time_log.time_out > lunch_start
                        )

                        if crosses_lunch:
                            # Deduct 1 hour for lunch break from total hours
                            time_log.hrs_rendered = round(
                                max(0, total_hrs_rendered - 1.0), 2
                            )
                        else:
                            time_log.hrs_rendered = round(total_hrs_rendered, 2)
                        ...
                        # Note: Lunch deduction does NOT apply to overtime hours
                        time_beyond_sched = (
                            time_log.time_out - sched_end_actual
                        ).total_seconds() / 3600
                        ...
                        if time_log.over_time > 0:
                            ot_hourly_rate = hourly_rate * 1.25
                            time_log.ot_pay = round(
                                ot_hourly_rate * time_log.over_time, 2
                            )
                        else:
                            time_log.ot_pay = 0.0
...
def scheduled_trips_report():
    ...
    try:
        ...
        start_date = datetime.strptime(start_date_str, "%Y-%m-%d").date()
        end_date = datetime.strptime(end_date_str, "%Y-%m-%d").date() + timedelta(
            days=1
        )
        ...
        schedules = (
            ...
            .order_by(Schedule.delivery_schedule)
            .all()
        )
        ...
        for schedule in schedules:
            for trip in schedule.trips:
                ...
                for detail in sorted_details:
                    ...
                    drivers = (
                        ", ".join([driver.name for driver in trip.drivers])
                        if trip.drivers
                        else "N/A"
                    )
                    ...
                    assistants = (
                        ", ".join([assistant.name for assistant in trip.assistants])
                        if trip.assistants
                        else "N/A"
                    )
                    ...
                    result.append(
                        {
                            "delivery_schedule": schedule.delivery_schedule.strftime(
                                "%Y-%m-%d"
                            ),
                            "trip_number": trip.trip_number,
...
def generate_scheduled_trips_report(start_date, end_date):
    ...
    for schedule in schedules:
        for trip in schedule.trips:
            for detail in trip.details:
                driver_name = (
                    ", ".join([d.name for d in trip.drivers]) if trip.drivers else "N/A"
                )
                assistant_name = (
                    ", ".join([a.name for a in trip.assistants])
                    if trip.assistants
                    else "N/A"
                )
...
def generate_cancelled_trips_report(start_date, end_date):
    ...
    cancelled_details = (
        db.session.query(TripDetail, Trip, Schedule)
        .join(Trip, TripDetail.trip_id == Trip.id)
        .join(Schedule, Trip.schedule_id == Schedule.id)
        .filter(
            Schedule.delivery_schedule >= start_date,
            Schedule.delivery_schedule <= end_date,
...
def generate_vehicle_utilization_report(start_date, end_date):
    ...
    trips = (
        db.session.query(Trip, Schedule)
        .join(Schedule, Trip.schedule_id == Schedule.id)
        .filter(
            Schedule.delivery_schedule >= start_date,
            Schedule.delivery_schedule <= end_date,
    ...
    for vehicle_id, stats in vehicle_stats.items():
        ...
        avg_cbm = (
            stats["total_cbm"] / stats["total_trips"] if stats["total_trips"] > 0 else 0
        )
...
def export_report():
    ...
    try:
        ...
        # Convert JSON to CSV
        df = pd.DataFrame(
            data.get_json().get("rows"), columns=data.get_json().get("headers")
        )
...
def truck_utilization():
    ...
    try:
        ...
        query = (
            ...
            .join(Trip, Schedule.id == Trip.schedule_id)
            .join(Vehicle, Trip.vehicle_id == Vehicle.id)
            .outerjoin(TripDetail, Trip.id == TripDetail.trip_id)
            .filter(
                Schedule.delivery_schedule >= start_date,
                Schedule.delivery_schedule <= end_date,
            )
            ...
            .order_by(Schedule.delivery_schedule, Trip.trip_number)
            .all()
        )
    ...
    try:
        ...
        for date in date_list:
            ...
            schedules_on_date = Schedule.query.filter(
                Schedule.delivery_schedule == date, Schedule.plate_number.isnot(None)
            ).all()
...
def export_truck_fleet_utilization():
    ...
    try:
        ...
        # Write headers
        writer.writerow(
            ["Date", "Day", "Trucks Used", "Active Trucks", "% Utilization"]
        )
        ...
        for date in date_list:
            ...
            utilization = (
                (trucks_used / active_trucks * 100) if active_trucks > 0 else 0
            )
...
@app.route("/api/manila_date")
@login_required
def api_manila_date():
    """Get current Manila date for frontend date pickers"""
    ...
    manila_date = manila_now.date()

    # Calculate 7 days ago
    from datetime import timedelta

    seven_days_ago = manila_date - timedelta(days=7)

    return jsonify(
        {"today": manila_date.isoformat(), "seven_days_ago": seven_days_ago.isoformat()}
    )
...
def fuel_efficiency_data():
    ...
    try:
        ...
        query = Odo.query.join(
            Vehicle, Odo.plate_number == Vehicle.plate_number
        ).filter(Odo.datetime >= start_date, Odo.datetime < end_date)
    ...
    try:
        ...
        for odo in odo_records:
            ...
            if odo.status == "start odo":
                if (
                    summary[plate]["start_odo"] is None
                    or odo_reading < summary[plate]["start_odo"]
                ):
                    summary[plate]["start_odo"] = odo_reading
            elif odo.status == "end odo":
                if (
                    summary[plate]["end_odo"] is None
                    or odo_reading > summary[plate]["end_odo"]
                ):
        ...
        for summary_row in summary.values():
            total_km = (
                summary_row["end_odo"] is not None
                and summary_row["start_odo"] is not None
            )
            total_km_value = (
                (summary_row["end_odo"] - summary_row["start_odo"])
                if total_km
                else None
            )

            km_per_liter = (
                summary_row["total_liters"] > 0 and total_km_value is not None
            )
            km_per_liter_value = (
                (total_km_value / summary_row["total_liters"]) if km_per_liter else None
            )

            avg_price_per_liter = (
                (summary_row["total_amount"] / summary_row["total_liters"])
                if summary_row["total_liters"] > 0
                else None
            )
            ...
            writer.writerow(
                [
                    ...
                    f"{km_per_liter_value:.2f}"
                    if km_per_liter_value is not None
                    else "N/A",
...
def frequency_rate():
    ...
    try:
        ...
        for area, data in sorted(
            area_data.items(), key=lambda x: len(x[1]["dates"]), reverse=True
        ):
            ...
            result.append(
                {"area": area, "delivery_count": delivery_count, "areas": sorted_dates}
            )
...
def export_frequency_rate():
    ...
    try:
        ...
        for area, data in sorted(
            ...
            writer.writerow([rank, area, delivery_count, dates_str])
            rank += 1
                ...
                for detail in trip.details:
                    ...
                    if detail.status in ("Delivered", "Cancelled"):
                        ...
                        if detail.original_due_date:
                            days_late = (
                                schedule.delivery_schedule - detail.original_due_date
                            ).days

                        # Calculate In Full: total_delivered_qty - total_ordered_qty
                        qty_diff = None
                        if (
                            detail.total_delivered_qty is not None
                            and detail.total_ordered_qty is not None
                        ):
                            qty_diff = (
                                detail.total_delivered_qty - detail.total_ordered_qty
                            )

                        result.append(
                            {
                                "scheduled_date": schedule.delivery_schedule.strftime(
                                    "%Y-%m-%d"
                                ),
                                "branch_name_v2": detail.branch_name_v2,
                                "total_ordered_qty": detail.total_ordered_qty or 0,
                                "original_due_date": detail.original_due_date.strftime(
                                    "%Y-%m-%d"
                                )
                                if detail.original_due_date
                                else "N/A",
                ...
                for detail in trip.details:
                    ...
                    if detail.status in ("Delivered", "Cancelled"):
                        ...
                        if (
                            ...
                            if qty_diff >= 0:
                                in_full_status = f"In Full" + (
                                    f" (+{qty_diff})" if qty_diff > 0 else ""
                                )
                            else:
                        ...
                        writer.writerow(
                            [
                                ...
                                detail.branch_name_v2,
                                detail.total_ordered_qty or 0,
                                detail.original_due_date.strftime("%Y-%m-%d")
                                if detail.original_due_date
                                else "N/A",
                                detail.total_delivered_qty or 0,
                                on_time_status,
...
def missing_data_report():
    ...
    try:
        ...
        missing_trips_query = (
            db.session.query(TripDetail, Trip, Schedule, Vehicle)
            .join(Trip, TripDetail.trip_id == Trip.id)
            .join(Schedule, Trip.schedule_id == Schedule.id)
            .join(Vehicle, Trip.vehicle_id == Vehicle.id)
            .filter(
            ...
            .options(joinedload(Trip.drivers), joinedload(Trip.assistants))
            .all()
        )
        ...
        for detail, trip, schedule, vehicle in missing_trips_query:
            ...
            driver_names = (
                [driver.name for driver in trip.drivers] if trip.drivers else []
            )
            assistant_names = (
                [assistant.name for assistant in trip.assistants]
                if trip.assistants
                else []
            )
            ...
            missing_trips.append(
                {
                    ...
                    "assistants": ", ".join(assistant_names)
                    if assistant_names
                    else "N/A",
                    "trip_number": trip.trip_number,
                    "branch_name": detail.branch_name_v2 or "N/A",
                    "arrive": detail.arrive.strftime("%I:%M %p")
                    if detail.arrive
                    else "Missing",
                    "departure": detail.departure.strftime("%I:%M %p")
                    if detail.departure
                    else "Missing",
                }
            )
        ...
        # First, get all vehicles used in the date range from Schedule table
        # Schedule stores plate_number directly, so we query it to get all scheduled vehicles
        vehicles_used = (
            db.session.query(
                Schedule.delivery_schedule,
                Vehicle.id,
            ...
            .outerjoin(Trip, Trip.schedule_id == Schedule.id)
            .outerjoin(Vehicle, Vehicle.plate_number == Schedule.plate_number)
            .filter(
                Schedule.delivery_schedule >= start_date,
                Schedule.delivery_schedule < end_date,
                Schedule.plate_number != None,  # Only get schedules with plate numbers
                Schedule.plate_number != ''     # Exclude empty plate numbers
            )
            .distinct(Schedule.delivery_schedule, Schedule.plate_number)
            .all()
        )

        # Get all trips with drivers and assistants for the date range
        # This will help us show who was assigned to each vehicle
        trips_with_manpower = (
            db.session.query(Trip, Schedule)
            .join(Schedule, Trip.schedule_id == Schedule.id)
            .filter(
        ...
        # Build a mapping of (schedule_date, plate_number) -> (drivers, assistants)
        # We need to get the plate_number from the trip's vehicle relationship
        manpower_mapping = {}
        for trip, schedule in trips_with_manpower:
            # Get the plate number from the trip's vehicle
            plate_number = trip.vehicle.plate_number if trip.vehicle else None
            if not plate_number:
                continue

            key = (schedule.delivery_schedule, plate_number)
            if key not in manpower_mapping:
                manpower_mapping[key] = {'drivers': set(), 'assistants': set()}

            # Add driver names
            if trip.drivers:
                for driver in trip.drivers:
                    manpower_mapping[key]['drivers'].add(driver.name)

            # Add assistant names
            if trip.assistants:
                for assistant in trip.assistants:
                    manpower_mapping[key]['assistants'].add(assistant.name)

        # Get all ODO records in the date range (using datetime field)
        # Database-agnostic approach: compare datetime objects directly
        ...
        odo_records = (
            db.session.query(Odo)
            .filter(
                Odo.datetime >= start_dt,
                Odo.datetime < end_dt,
            )
            .all()
        )

        # Create a dictionary to track which ODO types exist for each (date, plate_number)
        # Structure: {(date, plate_number): {'start': bool, 'refill': bool, 'end': bool}}
        odo_coverage = {}
        for odo in odo_records:
            key = (odo.datetime.date(), odo.plate_number)
            if key not in odo_coverage:
                odo_coverage[key] = {'start': False, 'refill': False, 'end': False}

            # Mark which type of ODO record exists
            if odo.status == 'start odo':
                odo_coverage[key]['start'] = True
            elif odo.status == 'refill odo':
                odo_coverage[key]['refill'] = True
            elif odo.status == 'end odo':
                odo_coverage[key]['end'] = True

        # Find vehicles with missing ODO records
        missing_odo = []
        for schedule_date, vehicle_id, plate_number, dept in vehicles_used:
            key = (schedule_date, plate_number)

            # Get driver and assistant names from manpower mapping
            manpower = manpower_mapping.get(key, {'drivers': set(), 'assistants': set()})
            driver_names = sorted(manpower['drivers']) if manpower['drivers'] else []
            assistant_names = sorted(manpower['assistants']) if manpower['assistants'] else []

            # Check if this vehicle/date has any ODO records
            if key not in odo_coverage:
                # No ODO records at all for this vehicle on this date
                missing_odo.append(
                    {
                ...
                # Vehicle has some ODO records, but check if any types are missing
                odo_types = odo_coverage[key]
                # Only include if at least one type is missing
                if not all([odo_types['start'], odo_types['refill'], odo_types['end']]):
                    missing_odo.append(
                        {
                            "delivery_schedule": schedule_date.strftime("%Y-%m-%d"),
                            "plate_number": plate_number,
                            ...
                            "assistants": ", ".join(assistant_names) if assistant_names else "N/A",
                            "department": dept or "N/A",
                            "has_start_odo": odo_types['start'],
                            "has_refill_odo": odo_types['refill'],
                            "has_end_odo": odo_types['end'],
                        }
                    )

        result = {"missing_trips": missing_trips, "missing_odo": missing_odo}
...
def export_missing_data():
    ...
    try:
        ...
        for detail, trip, schedule, vehicle in missing_trips_query:
            ...
            writer.writerow(
                [
                    ...
                    detail.departure.strftime("%I:%M %p")
                    if detail.departure
                    else "Missing",
                ]
            )
        ...
        # Query vehicles used in date range from Schedule table
        vehicles_used = (
            db.session.query(
                Schedule.delivery_schedule,
                Vehicle.id,
        ...
        for schedule_date, vehicle_id, plate_number, dept in vehicles_used:
            ...
            if key not in odo_coverage:
                ...
                # Vehicle has some ODO records, check if any types are missing
                odo_types = odo_coverage[key]
                if not all([odo_types['start'], odo_types['refill'], odo_types['end']]):
                    writer.writerow(
                        [
                            ...
                            ", ".join(assistant_names) if assistant_names else "N/A",
                            dept or "N/A",
                            "Yes" if odo_types['start'] else "No",
                            "Yes" if odo_types['refill'] else "No",
                            "Yes" if odo_types['end'] else "No",
                        ]
                    )
...
def get_time_log_matrix_data(start_date, end_date):
    ...
    manpower_assignments = (
        ...
        .order_by(Manpower.name, Manpower.role)
        .all()
    )
    ...
    # Generate list of all dates in range (store both obj and str)
    date_list = []
    current_date = start_date
    while current_date < end_date:
        date_list.append(
            {"obj": current_date, "str": current_date.strftime("%Y-%m-%d")}
        )
        current_date += timedelta(days=1)
    ...
    for person in unique_personnel.values():
        ...
        for date_info in date_list:
            date_obj = date_info["obj"]
            date_str = date_info["str"]

            if person.user_id is None:
                # No user account linked
                dates_dict[date_str] = {"time_in": "Missing", "time_out": "Missing"}
            else:
    ...
    return personnel_list, [d["str"] for d in date_list]
...
def search_trip_details():
    ...
    try:
        ...
        trip_details = (
            db.session.query(TripDetail, Schedule)
            .join(Trip, TripDetail.trip_id == Trip.id)
            .join(Schedule, Trip.schedule_id == Schedule.id)
            .filter(TripDetail.document_number == document_number)
            .order_by(Schedule.delivery_schedule.desc())
            .all()
        )
...
def search_data_records():
    ...
    try:
        ...
        data_records = (
            Data.query.filter(Data.document_number == document_number)
            .order_by(Data.item_number)
            .all()
        )
            ...
            result.append(
                {
                    ...
                    "branch_name": record.branch_name,
                    "branch_name_v2": record.branch_name_v2,
                    "original_due_date": record.original_due_date.strftime("%Y-%m-%d")
                    if record.original_due_date
                    else None,
                }
            )
...
def search_backload():
    ...
    try:
        ...
        backloads = (
            Backload.query.filter(Backload.document_number == document_number)
            .order_by(Backload.created_at.desc())
            .all()
        )
        ...
        for backload in backloads:
            result.append(
                {
                    "id": backload.id,
                    "created_at": backload.created_at.strftime("%Y-%m-%d %H:%M:%S")
                    if backload.created_at
                    else None,
    ...
    try:
        ...
        if backload_qty is None or backload_qty <= 0:
            return jsonify(
                {
                    "success": False,
                    "message": "Backload quantity must be greater than zero",
                }
            ), 400

        if not backload_remarks:
            return jsonify(
                {"success": False, "message": "Backload remarks are required"}
            ), 400
        ...
        if backload_qty > data_record.delivered_qty:
            return jsonify(
                {
                    "success": False,
                    "message": "Backload quantity cannot exceed delivered quantity",
                }
            ), 400
        ...
        for td in trip_details:
            if td.data_ids:
                ...
                if str(record_id) in data_ids:
                    ...
                    td.total_delivered_qty = (
                        td.total_delivered_qty or 0
                    ) - backload_qty
        ...
        db.session.rollback()
        return jsonify(
            {"success": False, "message": f"Error creating backload record: {str(e)}"}
        ), 500
        ...
        try:
            ...
            for row_num, row in enumerate(all_rows, start=2):
                try:
                    ...
                    isms_upload_date = (
                        parse_date_flexible(row["ISMS Upload Date"])
                        if row["ISMS Upload Date"]
                        else None
                    )
                    delivery_date = (
                        parse_date_flexible(row["Delivery Date"])
                    ...
                    if email:
                        ...
                        if not re.match(email_pattern, email):
                            raise ValueError(
                                f"Invalid email format: '{email}'. Email must be in valid format (e.g., user@example.com)"
                            )
            ...
            csv_keys = [
                (r["sap_upload_date"], r["customer_name"], r["serial_number"])
                for r in validated_records
            ]
            ...
            if records_to_insert:
                try:
                    ...
                    with db.session.no_autoflush:
                        for record in records_to_insert:
                            lcl_detail = LCLDetail(
                                ...
                                email=record[
                                    "email"
                                ],  # Store email from CSV for visibility
                            )
                    ...
                    summary_data = defaultdict(
                        lambda: {"qty": 0, "cbm": 0.0, "email": None}
                    )
                    ...
                    for (posting_date, branch_name), totals in summary_data.items():
                        ...
                        if key in existing_summaries:
                            ...
                            summary = LCLSummary(
                                ...
                                team_lead=totals[
                                    "email"
                                ],  # Set team_lead from detail email
                    ...
                    message = (
                        f"Successfully uploaded {records_added} LCL detail record(s)!"
                    )
                    total_skipped = records_skipped + csv_internal_skipped
                    if total_skipped > 0:
                        if csv_internal_skipped > 0:
                ...
                if csv_internal_skipped > 0:
                    message += (
                        f" (skipped {csv_internal_skipped} duplicate(s) within CSV)."
                    )
                else:
...
def edit_lcl_summary(id):
    ...
    if request.method == "POST":
        try:
            ...
            summary.length_width_height = (
                request.form.get("length_width_height") or None
            )
            ...
            summary.port_of_destination = (
                request.form.get("port_of_destination") or None
            )
            ...
            summary.actual_delivery_leadtime = parse_int_field(
                "actual_delivery_leadtime"
            )
            summary.received_date_to_pick_up_date = parse_int_field(
                "received_date_to_pick_up_date"
            )
...
def daily_vehicle_counts():
    ...
    return render_template(
        "daily_vehicle_counts.html", counts=counts, scheduler_running=scheduler_running
    )
...
def run_vehicle_count():
    ...
    if success:
        ...
        return jsonify(
            {
                "success": False,
                "message": "Failed to count vehicles. Check server logs.",
            }
        )
...
def edit_daily_vehicle_count(id):
    ...
    try:
        ...
        existing = DailyVehicleCount.query.filter(
            DailyVehicleCount.date == new_date, DailyVehicleCount.id != id
        ).first()

        if existing:
            flash(
                f"A record for {new_date.strftime('%B %d, %Y')} already exists.",
                "error",
            )
            return redirect(url_for("daily_vehicle_counts"))
...
def export_daily_vehicle_counts():
    ...
    try:
        ...
        for count in counts:
            ...
            recorded_at = (
                count.created_at.strftime("%Y-%m-%d %H:%M:%S")
                if count.created_at
                else "N/A"
            )

            writer.writerow(
                [count.date.strftime("%Y-%m-%d"), day_name, count.qty, recorded_at]
            )
    ...
    with app.app_context():
        try:
            ...
            active_count = Vehicle.query.filter_by(
                status="Active", dept="Logistics", type="in-house"
            ).count()
            ...
            if existing_count:
                ...
                db.session.commit()
                print(
                    f"[{datetime.now()}] Updated daily vehicle count for {today}: {active_count} active Logistics in-house vehicles"
                )
            else:
                # Create new record
                try:
                    ...
                    db.session.commit()
                    print(
                        f"[{datetime.now()}] Created daily vehicle count for {today}: {active_count} active Logistics in-house vehicles"
                    )
                except IntegrityError:
                    # Handle race condition - another process created the record
                    db.session.rollback()
                    existing_count = DailyVehicleCount.query.filter_by(
                        date=today
                    ).first()
                    if existing_count:
                        existing_count.qty = active_count
                        db.session.commit()
                        print(
                            f"[{datetime.now()}] Race condition handled - updated daily vehicle count for {today}: {active_count} active vehicles"
                        )
                    else:
                        print(
                            f"[{datetime.now()}] Error: Could not create or update daily vehicle count for {today}"
                        )
                        return False
...
def archive_status():
    ...
    recent_archives = (
        ArchiveLog.query.order_by(ArchiveLog.executed_at.desc()).limit(5).all()
    )
