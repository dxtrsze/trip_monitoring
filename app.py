...
def dashboard_comparisons():
    ...
    def calculate_person_performance(person_name, role, start_date, end_date):
        ...
        if role == 'driver':
            trip_ids = db.session.query(Trip.id).join(
                trip_driver, Trip.id == trip_driver.c.trip_id
            ).join(Manpower, Manpower.id == trip_driver.c.manpower_id).filter(
            ...
            ).join(Manpower, Manpower.id == trip_assistant.c.manpower_id).filter(
                Manpower.name == person_name,
                Schedule.delivery_schedule.between(start_date, end_date)
            ).all()
        ...
        trip_details = db.session.query(TripDetail).filter(
            TripDetail.trip_id.in_(trip_ids)
        ).all()

        total_drops = len(trip_details)
        completed_drops = sum(1 for td in trip_details if td.arrive and td.departure)
        drop_completion_rate = (completed_drops / total_drops * 100) if total_drops > 0 else 0.0

        # 2. Calculate ODO Compliance Rate
        # Get unique dates and vehicles for this person's trips
        trip_info = db.session.query(
            Schedule.delivery_schedule,
            Vehicle.plate_number
        ).join(Trip, Schedule.id == Trip.schedule_id).join(
            Vehicle, Trip.vehicle_id == Vehicle.id
        ).filter(
        ...
        for trip_date, plate_number in trip_info:
            ...
            odo_records = db.session.query(Odo.status).filter(
        ...
        odo_compliance_rate = (compliant_days / total_active_days * 100) if total_active_days > 0 else 0.0
    ...
    driver_names = db.session.query(Manpower.name).join(
        trip_driver, Manpower.id == trip_driver.c.manpower_id
    ).join(Trip, Trip.id == trip_driver.c.trip_id).join(
        Schedule
    ).filter(
        Schedule.delivery_schedule.between(start_date, end_date)
    ).distinct().all()

    assistant_names = db.session.query(Manpower.name).join(
        trip_assistant, Manpower.id == trip_assistant.c.manpower_id
    ).join(Trip, Trip.id == trip_assistant.c.trip_id).join(
        Schedule
    ).filter(
        Schedule.delivery_schedule.between(start_date, end_date)
    ).distinct().all()
